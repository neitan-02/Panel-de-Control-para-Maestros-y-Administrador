...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const register = async (data: RegisterData): Promise<{ success: boolean; message: string }> => {
    try {
      const response = await axios.post("http://localhost:5000/api/maestros/register", data);
  ...
  const login = async (data: LoginData): Promise<Maestro> => {
    try {
      const res = await axios.post("http://localhost:5000/api/maestros/login", data);
      ...
      const userRes = await axios.get("http://localhost:5000/api/maestros/me", {
